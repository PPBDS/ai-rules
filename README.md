...
| [`claude-md/vscode/CLAUDE.md`](claude-md/vscode/CLAUDE.md) *(pointer)* | [vscode.tutorials](https://github.com/PPBDS/vscode.tutorials) | The infrastructure / mechanics package — the first tutorials students do, which teach the tools the base guide *assumes* (Git, GitHub, Codespaces, Quarto, terminals, CP/CR). It is the **exception** the base guide does not govern. Its guide is fully package-specific, so the **canonical file lives in the [vscode.tutorials repo](https://github.com/PPBDS/vscode.tutorials/blob/main/CLAUDE.md)** as that repo's own `CLAUDE.md`; the file here is a pointer to it. |
