...
- **Caching is its own exercise — never combined with another step.** Adding `#| cache: true` is a *refactoring* discipline, taught on its own. The natural pattern is two consecutive exercises: the first creates and uses the artifact (function, fitted model, expensive tibble) in a single working chunk, no caching; the second is a pure refactor — pull the artifact's definition out into a new chunk dedicated to it, add `#| cache: true`, and leave only the usage in the working chunk. Do not fold "add `#| cache: true`" into "build this object" or "copy our code" prompts — the caching question stands alone.
