...
**Normal vs modeling tutorials.** Within the tutorials this guide governs, there is one recognized split. Most are **normal** tutorials — all of `misc.tutorials`, for instance — where we mostly don't care which code produces which result: students steer AI and judge the *output*. The Primer's example tutorials (tutorial 05 and higher in `primer.tutorials`) are **modeling** tutorials: there, especially in the first five or so, we *want* students to see — perhaps even type — the modeling code (`reg_linear()`, `plot_predictions()`), and questions deliberately use tools like `Cmd/Ctrl + Enter` and `Cmd/Ctrl + Shift + K` so students slow down and **look** at which code produces which result. That inverted emphasis is recorded in the Primer's own guide; this base guide describes the normal case.
...
- Render + Live Server is the **primary** interaction methodology. `Cmd/Ctrl + Enter` — running a line of QMD code in the R Terminal — is sometimes useful, and sometimes worth teaching, especially when students are first learning a new skill. But use it deliberately, not as the routine way to see output; when a tutorial is not teaching a brand-new topic, stick to render + Live Server. Rendering itself is usually delegated — students mostly tell AI to render (or run `quarto render`) — but rendering by hand with `Cmd/Ctrl + Shift + K` is fine too.
...
Showing our code *and* its result, via `echo = TRUE`, is the **default for every answer** — we often show students our code precisely so they learn something from comparing it with their own. (This is about *our* code in the tutorial: the student's own rendered document always hides its code via `execute: echo: false`, because readers of an analysis want the graphics, not the code.) In a normal tutorial, though, the *output* is the real object of comparison — we mostly care that the student's result is right, not that their code matches ours; modeling tutorials invert this emphasis (see §1). A plain three-backtick block (code only) is an acceptable fallback, and the right choice when the code would take more than ~5 seconds to run, but the live `echo = TRUE` chunk is preferred.
